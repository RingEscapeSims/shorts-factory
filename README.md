...
## Engine 2: Kids Studio (Made for Kids — read this part carefully)

`kids_studio.py` makes original educational cartoons (counting, colors; more
templates via CLAUDE.md). Key commands:

```bash
python kids_studio.py --mode counting --seed 7    # test video
python kids_studio.py --setup-voice               # natural free voice (Piper)
python daily_run.py --count 2 --engine kids       # a day of kids content
```

Rules that are different from the rings engine:

- Every video is auto-labeled **Made for Kids** (`selfDeclaredMadeForKids:
  true`). This is a COPPA legal requirement for child-directed content —
  never change it. Consequences of the label: comments and notifications
  are disabled, only contextual (non-personalized) ads run, and RPM is much
  lower than general content. The upside: eligibility for the YouTube Kids
  app audience.
- **Use a separate channel** for kids content (add a new channel on the same
  Google account; pick it during the OAuth screen). Mixing kids + neon
  rings confuses the algorithm and the audience.
- Voice: the built-in fallback is robotic. Run `--setup-voice` once (free,
  local Piper TTS) — voice quality is the biggest quality lever. Check the
  downloaded voice's MODEL_CARD license permits commercial use.
- Quality bar: YouTube demonetizes and can suspend channels focused on
  low-quality made-for-kids content, and boosts content that teaches.
  Every video must teach one real thing. Two good videos beat six lazy ones
  — nowhere is that more enforced than kids content.
