...
def schedule():
    ...
    eng = input("Which content? [1] neon rings  [2] kids learning "
                "[3] alternate both  (default 2): ").strip()
    eng = {"1": "rings", "2": "kids", "3": "mix"}.get(eng, "kids")
    ans = input(f"Register 2 uploads/day of '{eng}' (09:00 and 18:00)? "
                "Recommended starting volume. [Y/n] ").strip().lower()
    if ans == "n":
    ...
    if platform.system() == "Windows":
        for name, t in [("ShortsFactoryAM", "09:00"),
            ...
            run(["schtasks", "/Create", "/F", "/SC", "DAILY",
                 "/TN", name, "/ST", t,
                 "/TR", f'"{py}" "{script}" --count 1{extra}'])
        print("\nDone. Verify in Task Scheduler; if a task misfires, edit its")
        print(f'Action to: {py} "{script}" --count 1{extra}')
    else:
        lines = (f'0 9 * * * cd "{HERE}" && "{py}" daily_run.py --count 1{extra}\n'
                 f'0 18 * * * cd "{HERE}" && "{py}" daily_run.py --count 1{extra}\n')
